...
> [!IMPORTANT]
> 任务定义与实验协议以 `docs/TASK_DEFINITION.md` 为唯一准则；本文档偏方法论叙述，若与其冲突以其为准。
...
模型要学习的是：**条件轨迹分布**。但需要明确版本边界：

- **v1（当前代码）**：建模对象是**窗口级未来段生成（带观测历史）**，即学习
  $$P(\\mathrm{vel}_{t+1:t+F} \\mid \\mathrm{obs}_{t-H+1:t}, o, d, t_0, \\text{env})$$
  其中 `vel` 语义为 **step displacement**（步位移），位置通过积分得到（见 `docs/TASK_DEFINITION.md`）。
- **v2（未来）**：才扩展到“整段 trip 的 one-shot / autoregressive 生成”与 road-level 评估（map-matching）。

（下面的公式作为概念层统一符号，用于叙述方法论。）
...
| 起点 | $o$ | 起始道路/区域 ID 或坐标 | ✓ |
| 终点 | $d$ | 终止道路/区域 ID 或坐标 | ✓ |
| 出发时间 | $t_0$ | 小时 + weekday（v1）；weekend/sin-cos（可选 v2） | ✓ |
