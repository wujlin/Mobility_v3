...
\begin{figure}[t]
  ...
  \caption{Framework overview. A frozen deterministic prior captures OD-driven macroscopic motion scale. A diffusion model samples stochastic residual deviations, optionally conditioned on a train-only navigation-field prior. The sum yields diverse trajectories while preserving macroscopic motion statistics.}
...
To incorporate an empirical flow prior without relying on a road graph, we construct a train-only \emph{navigation field} that summarizes local mean displacement direction and speed in grid space. For each grid cell we estimate a mean direction $\mathbf{u}_{ij}$ and mean speed $s_{ij}$ from the training split. At inference, we crop a local patch around the last observed position $\mathbf{p}_0$:
\begin{equation}
  \Phi(\mathbf{p}_0) \in \mathbb{R}^{3\times K\times K}, \quad
  \Phi = [\,\mathbf{u}_y,\mathbf{u}_x, s\,],
  \label{eq:nav_patch}
\end{equation}
encode it with a CNN encoder, and concatenate the resulting embedding with the global condition.
...
\end{equation}
where $f_{\mathrm{base}}$ is a frozen baseline trained under the same dt-fixed protocol. During training, we define the diffusion target as the residual velocity $\mathbf{v}^{\mathrm{res}}=\mathbf{v}-\mathbf{v}^{\mathrm{prior}}$. During inference, we sample $\mathbf{v}^{\mathrm{res}}$ and add it back to the prior to obtain the final future trajectory.
