...
Per-city, Columbus exhibits a stronger signal (mean AUC $\approx 0.84$ over 18 groups) while Detroit is weaker but still above chance (mean AUC $\approx 0.61$ over 4 groups), highlighting the role of data volume and OD repetition.
...
\begin{table}[t]
  ...
  \begin{tabular}{lccc}
    ...
    Detroit+Columbus & 22 & 0.802 & STRONG\_GO \\
    \bottomrule
  \end{tabular}
...
In the multi-city setting, we found that a naive ``bin$\rightarrow$node'' instantiation can fail catastrophically by mixing nodes across disjoint city subgraphs that share a normalized grid coordinate system.
After enforcing city-consistent decoding and a simple reachability-friendly bin instantiation heuristic (major-road preference + OD-aligned direction), waypoint AR + A* achieves near-perfect completion.
However, an oracle experiment that replaces predicted bins with GT-derived bins shows only a small gain in corridor match, indicating that the remaining bottleneck is the coarse bin granularity rather than the bin predictor.
...
\begin{table}[t]
  ...
  \begin{tabular}{lccc}
    ...
    Waypoint AR + A* (T4) & 1.000 & 0.270 & city-consistent + reachable instantiation \\
    Waypoint AR + A* (T4, oracle bins) & 1.000 & 0.276 & ceiling limited by bin granularity \\
    \bottomrule
  \end{tabular}
  \caption{Graph corridor generation on the combined Detroit+Columbus dataset (200 sampled routes, $K=20$). Success denotes completion to $d$; $J_{\mathrm{best}}$ is best-of-$K$ edge Jaccard to a \emph{single} GT corridor for that route (single-trajectory match).}
  \label{tab:t3-t4}
\end{table}
