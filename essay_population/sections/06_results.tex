...
On the full split, our generated routes better match the ground-truth distributions of path length, detour factor (relative to straight-line distance), and per-route on-road rate than end-to-end diffusion.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/fig_realism_validation.pdf}
  \caption{Realistic route generation validation (Detroit). \textbf{(a)} $K=20$ samples for three fixed OD cases overlaid on an OSM-derived road-probability basemap. \textbf{(b)} Distribution alignment for path length, detour factor (vs.\ straight-line distance), and per-route on-road rate (fraction of points with road probability $\ge 0.5$).}
  \label{fig:realism}
\end{figure}
