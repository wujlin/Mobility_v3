...
def _plot_detour_hist(len_ratio: np.ndarray, out_png: Path) -> None:
    x = np.asarray(len_ratio, dtype=np.float64)
    x = x[np.isfinite(x)]
    if x.size == 0:
        return
    # Robust clipping for a readable histogram.
    ...
    ax.set_ylabel("Count")
    ax.set_title("Detroit (WorldTrace) detour ratio distribution")
...
def _plot_detour_by_hour(len_ratio: np.ndarray, hour: np.ndarray, out_png: Path) -> None:
    ...
    ax.set_xlabel("Local hour (America/Detroit)")
...
def _plot_route_choice_heatmaps(
    ...
    *,
    ...
    for ax, p, heat in zip(axes, pairs, heatmaps):
        ...
        # start/end mean markers (in pixel coords)
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Detroit preliminary story analysis (detour distribution + OD route choice patterns).")
...
def main() -> None:
    ...
    detour_json = out_dir / "detour_ratio.json"
    detour_png = out_dir / "detour_ratio_hist.png"
    detour_by_hour_png = out_dir / "detour_ratio_by_hour.png"

    _plot_detour_hist(len_ratio_np, detour_png)
    _plot_detour_by_hour(len_ratio_np, hour_np, detour_by_hour_png)

    # route choice patterns (top OD buckets)
    ...
    out = {
        ...
        "detroit_grid": {"H": H, "W": W, "bbox": grid.bbox.__dict__, "res_m": {"x": res_x_m, "y": res_y_m}},
        ...
        "by_hour": {
            ...
            "hour_valid_n": int(np.sum(hour_np >= 0)),
        },
        "route_choice": {
            "od_bins": od_bins,
            "top_od": top_od,
        ...
        "artifacts": {
            "detour_ratio_json": str(detour_json),
            "detour_ratio_hist_png": str(detour_png),
    ...
    detour_json.write_text(json.dumps(out, ensure_ascii=False, indent=2))
    route_json.write_text(json.dumps({"picked": picked}, ensure_ascii=False, indent=2))
    if not args.quiet:
