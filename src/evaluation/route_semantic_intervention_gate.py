...
from src.evaluation.route_mode_collapse_metrics import Config as CollapseCfg
from src.evaluation.route_mode_collapse_metrics import run_metrics as run_collapse_metrics
...
def _apply_grid_patch_override(patch: np.ndarray, *, mode: str, seed: int) -> np.ndarray:
    ...
    if mode == "shuffle":
        ...
        n, c, s, _ = patch.shape
        flat = patch.reshape(n, c, s * s)
        perm = rng.permutation(int(s * s)).astype(np.int64, copy=False)
        flat = flat[:, :, perm]
        return flat.reshape(n, c, s, s).astype(np.float32, copy=False)
    raise ValueError(f"Unknown grid override: {mode} (expected one of none|zeros|shuffle|flip_lr)")
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Gate: semantic intervention test for decision models on a fixed case.")
    p.add_argument("--checkpoint", type=str, required=True, help="Decision checkpoint (route_wp_diffusion_rel_npz).")
    p.add_argument("--gt_report_json", type=str, required=True, help="GT report.json from route_gt_baseline.py (contains selected_cases).")
    p.add_argument("--case_npz", type=str, required=True, help="case_XX/gt_case.npz from route_gt_baseline.py.")
    p.add_argument("--semantic_dir", type=str, default=None, help="Semantic raster directory (required if checkpoint uses semantics).")
    p.add_argument("--out_dir", type=str, required=True)
    p.add_argument(
        "--interventions",
        type=str,
        default="none,zeros,shuffle,flip_lr",
        help="Comma-separated interventions over OD-aligned grid patch: none, zeros, shuffle, flip_lr.",
    )
    p.add_argument("--num_samples_per_condition", type=int, default=20)
    p.add_argument("--collapse_ratio", type=float, default=0.5)
    p.add_argument("--jacc_cell", type=float, default=8.0)
    p.add_argument("--seed", type=int, default=0)
    return p


def _parse_interventions(s: str) -> Tuple[str, ...]:
    items = [x.strip() for x in str(s).split(",") if str(x).strip()]
    if not items:
        raise ValueError("--interventions must be a non-empty comma-separated list.")
    return tuple(items)
...
def main() -> None:
    ...
    cfg = GateCfg(
        ...
        gt_report_json=str(args.gt_report_json),
        case_npz=str(args.case_npz),
        semantic_dir=(str(args.semantic_dir) if args.semantic_dir else None),
        out_dir=str(args.out_dir),
        interventions=_parse_interventions(str(args.interventions)),
    ...
    k_wp = int(ckpt_cfg.get("K_waypoints", 0) if isinstance(ckpt_cfg, dict) else 0)
    if k_wp <= 0:
        raise ValueError("Checkpoint missing K_waypoints")
    od_bin = float(ckpt_cfg.get("od_bin", 128.0))
    o_clip = float(ckpt_cfg.get("o_clip", 2.0))
    temporal_meta = ckpt_cfg.get("temporal", {}) if isinstance(ckpt_cfg, dict) else {}
    temporal_mode = "zeros"
    temporal_tz = -5.0
    ...
    rel_norm = ckpt_cfg.get("rel_norm", {}) if isinstance(ckpt_cfg, dict) else {}
    ...
    norm = _load_norm_from_ckpt(ckpt_cfg if isinstance(ckpt_cfg, dict) else {})

    case = load_route_windows_npz(str(cfg.case_npz), max_n=None, seed=int(cfg.seed))
    ...
    sem_cfg_raw = ckpt_cfg.get("semantic_od_norm") if isinstance(ckpt_cfg, dict) else None
    ...
    sem_meta = ckpt_cfg.get("semantic") if isinstance(ckpt_cfg, dict) else None
    ...
    if uses_semantics and sem_mode not in ("rand4", "gridcnn", "od_gridcnn", "gridpos", "od_gridpos", "gridattn", "od_gridattn") and cfg.semantic_dir is None:
        raise ValueError("--semantic_dir is required because checkpoint includes semantic features")
    if sem_mode in ("gridcnn", "od_gridcnn", "gridpos", "od_gridpos", "gridattn", "od_gridattn") and cfg.semantic_dir is None:
    ...
    # Precompute vector semantics (same for all interventions).
    cond_vec = base_cond
    if sem_mode is not None and sem_mode in ("rand4", "od", "od_profile", "od_grid", "profile", "grid", "od_gridcnn", "od_gridpos", "od_gridattn"):
        if sem_cfg is None:
        ...
        cond_vec = np.concatenate([cond_vec, sem_norm.astype(np.float32, copy=False)], axis=1).astype(np.float32, copy=False)

    # Grid-CNN semantics: build patch once, apply interventions per-run, re-encode.
    grid_patch_norm = None
    grid_norm = None
    grid_encoder_state = None
    if sem_mode in ("gridcnn", "od_gridcnn", "gridpos", "od_gridpos", "gridattn", "od_gridattn"):
        grid_norm_raw = ckpt_cfg.get("semantic_grid_norm") if isinstance(ckpt_cfg, dict) else None
        ...
        grid_encoder_state = ckpt.get("grid_encoder_state_dict")
        ...
        grid_patch_norm = normalize_grid_patch(grid_patch_raw, grid_norm)
    ...
    posenc = None
    attn_control = None
    if grid_patch_norm is not None:
        if sem_mode in ("gridcnn", "od_gridcnn"):
            grid_encoder = GridCNNEncoder(in_channels=int(grid_patch_norm.shape[1]), out_dim=int(grid_emb_dim)).to(device=device)
            if grid_encoder_state is None or not isinstance(grid_encoder_state, dict):
                raise ValueError("Checkpoint includes gridcnn but missing grid_encoder_state_dict.")
            grid_encoder.load_state_dict(grid_encoder_state)
            grid_encoder.eval()
        elif sem_mode in ("gridpos", "od_gridpos"):
            ...
            posenc = WaypointSemanticPosEnc(
                in_channels=int(grid_patch_norm.shape[1]),
                num_waypoints=int(k_wp),
                extent=float(grid_extent),
                rel_mean=torch.from_numpy(rel_mean).to(device=device, dtype=torch.float32),
                rel_std=torch.from_numpy(rel_std).to(device=device, dtype=torch.float32),
    ...
    for idx, mode in enumerate(cfg.interventions):
        run_dir = out_dir / f"intervention_{mode}"
        run_dir.mkdir(parents=True, exist_ok=True)
        out_npz = run_dir / "samples.npz"
        metrics_json = run_dir / "mode_collapse_metrics.json"
        sample_summary = run_dir / "sample_summary.json"

        cond = cond_vec
        if grid_patch_norm is not None:
            patch_run = _apply_grid_patch_override(grid_patch_norm, mode=str(mode), seed=int(cfg.seed) + 10007 + int(idx))
            if grid_encoder is not None:
                with torch.no_grad():
                    emb = grid_encoder(torch.from_numpy(patch_run).to(device=device, dtype=torch.float32)).detach().cpu().numpy().astype(np.float32, copy=False)
                cond = np.concatenate([cond_vec, emb], axis=1).astype(np.float32, copy=False)
            else:
                # gridpos: patch is injected via cond_emb_extra_fn; keep cond as vector-only.
                cond = cond_vec

        if int(cond.shape[1]) != int(cond_dim):
            raise ValueError(f"cond_dim mismatch: ckpt={cond_dim} vs built={cond.shape[1]} (mode={mode})")
        cond_t = torch.from_numpy(cond).to(device=device, dtype=torch.float32)

        k_samples = int(cfg.num_samples_per_condition)
        preds_k = np.zeros((n, k_samples, f, 2), dtype=np.float32)

        patch_t = None
        start_pos_t = None
        dest_pos_t = None
        if grid_patch_norm is not None and (posenc is not None or attn_control is not None):
            patch_run = _apply_grid_patch_override(grid_patch_norm, mode=str(mode), seed=int(cfg.seed) + 10007 + int(idx))
            patch_t = torch.from_numpy(patch_run).to(device=device, dtype=torch.float32)
            if posenc is not None:
                start_pos_t = torch.from_numpy(start_pos).to(device=device, dtype=torch.float32)
                dest_pos_t = torch.from_numpy(dest_pos).to(device=device, dtype=torch.float32)

        with torch.no_grad():
            for kk in tqdm(range(k_samples), desc=f"sample[{mode}]", dynamic_ncols=True):
                torch.manual_seed(int(cfg.seed) + 1000 + int(kk))
                if posenc is not None:
                ...
                # Postprocess: s in [0,1], o clipped; sort by s.
        ...
        sample_summary.write_text(
            json.dumps(
                {
                    "inputs": {"checkpoint": str(ckpt_path.resolve()), "case_npz": str(Path(cfg.case_npz).resolve())},
                    "config": {"intervention": str(mode), "K_waypoints": int(k_wp), "K_samples": int(k_samples), "seed": int(cfg.seed)},
        ...
        # Metrics: reuse GT report (selected_cases) and evaluate matched windows in this case npz.
        metrics = run_collapse_metrics(
            gt_report_json=Path(cfg.gt_report_json),
            gt_windows_npz=Path(cfg.case_npz),
            model_samples_npz=out_npz,
            out_json=metrics_json,
            cfg=CollapseCfg(jacc_cell=float(cfg.jacc_cell), collapse_ratio=float(cfg.collapse_ratio), seed=int(cfg.seed)),
        )
        metrics_json.write_text(json.dumps(metrics, ensure_ascii=False, indent=2), encoding="utf-8")

        per_run.append(
            {
                "intervention": str(mode),
                "paths": {"dir": str(run_dir.resolve()), "samples_npz": str(out_npz.resolve()), "metrics_json": str(metrics_json.resolve())},
    ...
    report = {
        "inputs": {
            ...
            "gt_report_json": str(Path(cfg.gt_report_json).resolve()),
            "case_npz": str(Path(cfg.case_npz).resolve()),
            "semantic_dir": (str(Path(cfg.semantic_dir).resolve()) if cfg.semantic_dir else None),
        },
        "config": {
            "interventions": list(cfg.interventions),
            "num_samples_per_condition": int(cfg.num_samples_per_condition),
            "collapse_ratio": float(cfg.collapse_ratio),
            "jacc_cell": float(cfg.jacc_cell),
            "seed": int(cfg.seed),
        },
        "checkpoint_semantic_mode": (str(sem_mode) if sem_mode is not None else None),
        "runs": per_run,
    }
