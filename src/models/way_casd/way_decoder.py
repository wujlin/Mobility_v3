...
class WayDecoder(nn.Module):
    ...
    def sample_decode_batched_dense(
        self,
        *,
        way_embedder: nn.Module,
        latent_tokens: torch.Tensor,  # (B,L,d_model)
        ...
        start_way: torch.Tensor,  # (B,)
        dest_way: torch.Tensor,  # (B,)
        way_region: Optional[torch.Tensor] = None,
        region_seq: Optional[List[List[int]]] = None,
        region_adj: Optional[torch.Tensor] = None,
        region_constraint_mode: str = "strict",
        region_constraint_fallback: str = "unconstrained",
        max_len: Optional[int] = None,
        max_candidates: Optional[int] = None,
        ...
        anti_loop_penalty: float = 0.0,
        anti_loop_penalty_k: int = 4,
    ) -> Tuple[List[List[int]], List[List[torch.Tensor]], List[List[torch.Tensor]]]:
        """
        Like sample_decode_batched but returns per-step log_probs and entropies
        (as tensors, keeping autograd graph) instead of their sums, for dense-reward RL.

        Returns:
          paths:     list of B way-id sequences
          logp_steps: list of B lists, each [lp_0, lp_1, ...] per-step log-prob tensors
          ent_steps:  list of B lists, each [ent_0, ent_1, ...] per-step entropy tensors
        """
        max_len = int(max_len) if max_len is not None else int(self.cfg.max_len)
        B = int(latent_tokens.shape[0])
        device = latent_tokens.device
        ...
        # Per-step storage: list of B lists.
        logp_steps: List[List[torch.Tensor]] = [[] for _ in range(B)]
        ent_steps: List[List[torch.Tensor]] = [[] for _ in range(B)]
        ...
        for step_idx in range(int(max_len)):
            ...
            # Region constraint mask (identical to sample_decode_batched).
            if region_paths is not None and region_ptr is not None and way_region is not None:
                allow0 = torch.full((B2,), -1, dtype=torch.long, device=device)
                allow1 = torch.full((B2,), -1, dtype=torch.long, device=device)
                for i, bi in enumerate(keep_ids):
            ...
            # Temperature sampling.
            logits_t = logits / float(temp)
            dist_cat = torch.distributions.Categorical(logits=logits_t)
            j = dist_cat.sample()
            logp = F.log_softmax(logits_t, dim=-1)
            lp = logp[torch.arange(B2, device=device), j]
            ent = dist_cat.entropy()

            # Store per-step values (keep graph; avoid CPU sync here).
            for idx_i, bi in enumerate(keep_ids):
                logp_steps[int(bi)].append(lp[int(idx_i)])
                ent_steps[int(bi)].append(ent[int(idx_i)])
        ...
        return paths, logp_steps, ent_steps
