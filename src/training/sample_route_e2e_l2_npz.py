...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Sample a deterministic L2 baseline (SeqBaseline) on a fixed GT case npz (for Fig.1 collapse).")
    p.add_argument("--checkpoint", type=str, required=True)
    p.add_argument("--case_npz", type=str, required=True, help="case_XX/gt_case.npz from route_gt_baseline.py")
    p.add_argument("--out_dir", type=str, required=True)
    p.add_argument("--num_samples_per_condition", type=int, default=20, help="K (will be identical copies)")
...
def main() -> None:
    ...
    start_pos_norm = normalize_pos(start_pos, norm)
    dest_pos_norm = normalize_pos(dest_pos, norm)
    ...
    model = SeqBaseline(obs_dim=4, act_dim=2, cond_dim=6, hidden_dim=int(hidden_dim)).to(device=device)
    ...
    vel = denormalize_vel(vel_norm.detach().cpu().numpy(), norm)  # (N,F,2)
    pos = start_pos[:, None, :] + np.cumsum(vel, axis=1)  # (N,F,2)

    k = int(args.num_samples_per_condition)
    preds_k = np.repeat(pos[:, None, :, :], repeats=k, axis=1).astype(np.float32, copy=False)  # (N,K,F,2)
