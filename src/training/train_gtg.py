    ...
    *,
    ...
    for bi, batch in enumerate(loader):
        ...
        for t in range(int(K_eff)):
            ...
            logits = model.score_candidates(cur_way=cur, cand_way=cand_t, cand_mask=mask_t, cond_emb=cond_emb)
...
def main() -> None:
    p = argparse.ArgumentParser(description="Train S1 (simplified): GTG learned cost + next-hop supervision.")
    ...
    model = GTGCostNet(cfg=cfg).to(device)
