...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Train a deterministic end-to-end L2 baseline (SeqBaseline) from route windows npz (Detroit).")
...
def main() -> None:
    ...
    dataset = RouteWindowsVelDataset(
        data=data,
        norm=norm,
        ...
        torch.save(
            {
                ...
                "config": {
                    "task": "route_e2e_l2_npz",
                    "F": int(f),
                    "model": {"hidden_dim": int(cfg.hidden_dim), "obs_len": 1, "cond_dim": 6},
                    "temporal": {
