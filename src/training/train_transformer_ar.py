...
def _run_epoch(
    *,
    ...
    for bi, batch in enumerate(loader):
        ...
        way_in = way[:, :-1]
        way_next = way[:, 1:]
        h_all = model.forward_tokens(way_in, route_cond=rc)  # (B,K-1,D)

        total = torch.tensor(0.0, device=device)
        correct = torch.tensor(0.0, device=device)
        n_tok = torch.tensor(0.0, device=device)

        T = int(way_in.shape[1])
...
def main() -> None:
    p = argparse.ArgumentParser(description="Train B3: Transformer AR baseline (way-space, candidate CE).")
    ...
    model = WayTransformerAR(cfg=cfg).to(device)
