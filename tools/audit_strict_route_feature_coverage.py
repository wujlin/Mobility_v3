...
def main():
    ...
    # Load routes
    wr = np.load(str(args.way_routes_npz), allow_pickle=True)
    way_seq_flat = np.asarray(wr["way_seq_flat"], dtype=np.int64).reshape(-1)
    way_seq_ptr = np.asarray(wr["way_seq_ptr"], dtype=np.int64).reshape(-1)
    N = int(way_seq_ptr.size) - 1
    print(f"Loaded {N} routes, {way_seq_flat.size} total tokens")
    ...
    route_missing_frac = []
    for i in range(N):
        lo, hi = int(way_seq_ptr[i]), int(way_seq_ptr[i + 1])
    ...
    for rank, idx in enumerate(worst_idx):
        lo, hi = int(way_seq_ptr[idx]), int(way_seq_ptr[idx + 1])
        L = int(hi - lo)
